...
st.set_page_config(page_title="Predicted Mortality Explorer", layout="wide")
...
FRIENDLY = {
    ...
    "Insurance_Plan_Group": "Insurance Plan Group",
    "Issue_Age": "Issue Age",
    "Issue_Age_mod": "Issue Age (Modeled)",
    "Issue_Age_Group": "Issue Age Group",
    "Issue_Age_Band": "Issue Age Band",
    "Duration": "Duration",
    ...
    "Face_Band_Group": "Face Amount Group",
    "Issue_Year": "Issue Year",
    "Attained_Age": "Attained Age",
    "Attained_Age_mod": "Attained Age (Modeled)",
    ...
    "Predicted_Death_Count": "Predicted Deaths - Count",
    "Predicted_Death_Amount": "Predicted Deaths - Amount",
}
...
FIELD_NOTES = {
    ...
    "Insurance_Plan_Group": "Grouped plan used in the model diagnostics.",
    "Issue_Age": "Age at issue.",
    "Duration": "Policy duration.",
    "Duration_Band": "Validation duration band from the notebook.",
    ...
    "Face_Band_Group": "Grouped face amount band used in model diagnostics.",
    "Issue_Year": "Policy issue year.",
    "Issue_Age_Band": "Validation issue-age band from the notebook.",
    "Attained_Age": "Current age at observation.",
    "Attained_Age_Band": "Validation attained-age band from the notebook.",
    "SOA_Antp_Lvl_TP": "Anticipated level term period.",
    "SOA_Guar_Lvl_TP": "Guaranteed level term period.",
    ...
    "Predicted_Death_Count": "Predicted death count from the count model.",
    "Predicted_Death_Amount": "Predicted death claim amount from the amount model.",
}
...
AE_OPTIONS = [
    ...
    "P/E (Count)",
    "P/E (Amount)",
    "Actual Count Rate / 1000",
    "Expected Count Rate / 1000",
    "Predicted Count Rate / 1000",
    "Actual Amount Rate / 1000",
    "Expected Amount Rate / 1000",
    "Predicted Amount Rate / 1000",
]
...
PAGE_SPECS = [
    ...
    ("Issue Year", "Issue_Year", "Issue_Year"),
...
def builtin_data_candidates() -> List[str]:
    ...
    return [
        ...
        os.path.join(app_dir, "script", "model_outputs", "session4_scored_full_refit.parquet"),
        os.path.join(app_dir, "model_outputs", "session4_scored_full_refit.parquet"),
        os.path.join(cwd, "script", "model_outputs", "session4_scored_full_refit.parquet"),
        os.path.join(cwd, "model_outputs", "session4_scored_full_refit.parquet"),
    ]
...
def is_ae_metric_label(label: str) -> bool:
    s = str(label)
    return (
        ("A/E (Count" in s)
        or ("A/E (Amount" in s)
...
def is_year_metric_label(label: str) -> bool:
    s = str(label).strip()
    first = s.split(" | ")[0].strip()
    return first in {"Observation_Year", "Observation Year", "Issue_Year", "Issue Year"}


def format_number_for_display(x, pct: bool = False) -> str:
    if pd.isna(x):
        return ""
    try:
        x = float(x)
...
def format_pivot_for_display(df: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()
    ...
    for col in out.columns:
        ...
        if is_ae_metric_label(col):
            out[col] = out[col].map(lambda v: format_number_for_display(v, pct=True))
        elif not is_year_metric_label(col):
            out[col] = out[col].map(format_number_for_display)
...
def selected_display_to_raw_values(col: str, selected: List[str]) -> Tuple[bool, List[str]]:
    ...
    for x in selected:
        ...
        if col == "Age_Ind":
            ...
            mapped = REVERSE_FACE_BAND_MAP.get(x)
            if mapped is not None:
                raw_vals.extend([mapped, x])
            else:
                raw_vals.append(x)
        else:
            raw_vals.append(x)
...
def ae_requirements(metric: str) -> Tuple[str, str, float]:
    if metric == "A/E (Count)":
        return "Death_Count", "ExpDth_VBT2015_Cnt", 1.0
    if metric == "A/E (Amount)":
        return "Death_Claim_Amount", "ExpDth_VBT2015_Amt", 1.0
    if metric == "A/P (Count)":
        return "Death_Count", "Predicted_Death_Count", 1.0
    if metric == "A/P (Amount)":
        return "Death_Claim_Amount", "Predicted_Death_Amount", 1.0
    if metric == "P/E (Count)":
        return "Predicted_Death_Count", "ExpDth_VBT2015_Cnt", 1.0
    if metric == "P/E (Amount)":
        return "Predicted_Death_Amount", "ExpDth_VBT2015_Amt", 1.0
    if metric in {"Actual Count Rate / 1000", "Actual Rate / 1000"}:
        return "Death_Count", "Policies_Exposed", 1000.0
    if metric in {"Expected Count Rate / 1000", "Expected Rate / 1000"}:
        return "ExpDth_VBT2015_Cnt", "Policies_Exposed", 1000.0
    if metric in {"Predicted Count Rate / 1000", "Predicted Rate / 1000"}:
        return "Predicted_Death_Count", "Policies_Exposed", 1000.0
    if metric == "Actual Amount Rate / 1000":
        return "Death_Claim_Amount", "Amount_Exposed", 1000.0
    if metric == "Expected Amount Rate / 1000":
        return "ExpDth_VBT2015_Amt", "Amount_Exposed", 1000.0
    if metric == "Predicted Amount Rate / 1000":
        return "Predicted_Death_Amount", "Amount_Exposed", 1000.0
    raise KeyError(metric)


def ratio_rate_options_present(available_columns: List[str]) -> List[str]:
    available = set(available_columns)
    out: List[str] = []
    for metric in AE_OPTIONS:
        try:
            numerator_col, denominator_col, _ = ae_requirements(metric)
        except KeyError:
            continue
        if numerator_col in available and denominator_col in available:
            out.append(metric)
    return out
...
def build_pivot_base_duckdb(
    ...
    selected_ratio_rate_metrics = [m for m in unique_preserve_order(list(ratio_rate_metrics)) if m in AE_OPTIONS]
    raw_measure_cols: List[str] = []
    for metric in selected_ratio_rate_metrics:
        try:
            ncol, dcol, _ = ae_requirements(metric)
        except KeyError:
            continue
        raw_measure_cols.extend([ncol, dcol])

    for v in unique_preserve_order(raw_measure_cols):
        if v not in NUM_COLUMNS:
            continue
    ...
    if selected_ratio_rate_metrics:
        for metric in selected_ratio_rate_metrics:
            try:
                ncol, dcol, scale = ae_requirements(metric)
            except KeyError:
                continue
            if {ncol, dcol}.issubset(helper_aliases):
                g[metric] = scale * safe_div(g[helper_aliases[ncol]], g[helper_aliases[dcol]])
...
def summarize_for_chart_duckdb(
    ...
    if line_selects:
        ...
        for metric in ae_metrics:
            ...
            tmp["_value"] = scale * safe_div(g[num_name], g[den_name])
...
def infer_bar_axis_label(bar_cols: List[str]) -> str:
    if any(c in ("Death_Claim_Amount", "ExpDth_VBT2015_Amt", "Predicted_Death_Amount", "Amount_Exposed") for c in bar_cols):
        return "Bars (amount totals)"
    if any(c in ("Death_Count", "ExpDth_VBT2015_Cnt", "Predicted_Death_Count", "Policies_Exposed") for c in bar_cols):
        return "Bars (count / exposure totals)"
    return "Bars"


def infer_line_axis_label(line_metrics: List[str]) -> str:
    ratio_metrics = {m for m in AE_OPTIONS if "Rate / 1000" not in m}
    if line_metrics and all(m in ratio_metrics for m in line_metrics):
        return "Lines (ratio)"
    if line_metrics and all("Rate / 1000" in m for m in line_metrics):
        return "Lines (rate per 1000)"
    return "Lines"

def infer_line_tickformat(line_metrics: List[str]) -> str:
    ratio_metrics = {m for m in AE_OPTIONS if "Rate / 1000" not in m}
    return ".0%" if line_metrics and all(m in ratio_metrics for m in line_metrics) else ".4f"
...
def make_visual_figure(
    ...
    if chart_mode in ("Combo", "Bars only") and not bars.empty:
        for metric in bar_metrics:
            if metric not in bars.columns:
                continue
            hover_fmt = ":.0f" if metric in {"Observation_Year", "Issue_Year"} else ":,.4f"
            fig.add_trace(
                go.Bar(
                    x=bars["_axis_value"].astype(str),
                    y=bars[metric],
                    name=human(metric),
                    hovertemplate=f"{x_title}: %{{x}}<br>{human(metric)}: %{{y{hover_fmt}}}<extra></extra>",
                ),
                secondary_y=False,
            )
    ...
    if chart_mode in ("Combo", "Lines only") and not lines.empty:
        ...
        for trace_name in lines["_trace_label"].dropna().astype(str).unique().tolist():
            ...
            metric_name = str(
                sub["_line_metric"].iloc[0]) if "_line_metric" in sub.columns and not sub.empty else trace_name
            hover_fmt = ".2%" if metric_name.startswith(("A/E", "A/P", "P/E")) else ":,.4f"
            fig.add_trace(
                go.Scatter(
                    ...
                    mode=plot_mode,
                    name=trace_name,
                    hovertemplate=f"{x_title}: %{{x}}<br>{trace_name}: %{{y{hover_fmt}}}<extra></extra>",
                ),
    ...
    if chart_mode == "Combo":
        ...
        fig.update_yaxes(title_text=infer_line_axis_label(line_metrics), secondary_y=True, tickformat=infer_line_tickformat(line_metrics))
    elif chart_mode == "Bars only":
        fig.update_yaxes(title_text=infer_bar_axis_label(bar_metrics), tickformat=",.4g")
    else:
        fig.update_yaxes(title_text=infer_line_axis_label(line_metrics), tickformat=infer_line_tickformat(line_metrics))
...
def default_filter_config(path: str, nrows: Optional[int], available_columns: List[str]) -> Tuple[Dict[str, List[str]], Dict[str, Tuple[float, float]]]:
    ...
    if "Attained_Age" in available_columns:
        ...
        if lo is not None and hi is not None:
            default_num_ranges["Attained_Age"] = (float(max(lo, 1)), float(min(hi, 67)))
...
def render_welcome_tab(source_name: str, data_path: str, available_columns: List[str]) -> None:
    st.title("Predicted Mortality Experience Explorer")

    st.markdown(
        """
        **Basic flow**
        1. Use **Preliminary filters** to define the working data.
        2. Use **Pivot table** for cross-tab summaries with A/E, A/P, P/E, and rate output.
        3. Use the study tabs to build charts from the current preliminary working data.
        """
    )
    ...
    st.markdown(
        """
        **Predicted file**
        - The app reads `script/data/parquet/full.parquet`.
        - The file is the full mortality data with appended count and amount model predictions.
        - It includes `Predicted_Death_Count` and `Predicted_Death_Amount`; ratio and rate diagnostics are recomputed from aggregated totals.
        """
    )
    ...
    st.markdown(
        ...
        **Notes**
        ...
        - Ratio metrics are recomputed from aggregated totals so segment-level A/E and A/P stay actuarially meaningful.
        """
    )
...
def render_preliminary_filters_tab(
    ...
    st.caption("These filters define the working dataset used by the pivot table and study tabs.")
...
def render_pivot_tab(
    ...
    st.caption("Build a pivot table from the current preliminary working data.")
    ...
    ratio_rate_fields = ratio_rate_options_present(available_columns)

    with st.form("pivot_form"):
        c1, c2, c3, c4 = st.columns([1.1, 1.1, 1.1, 0.9], gap="large")
        with c1:
            row_fields = st.multiselect(
                "Rows",
                axis_fields,
                default=[c for c in ["Sex"] if c in axis_fields],
                key="pivot_rows_duck",
            )
        with c2:
            col_fields = st.multiselect(
                "Columns",
                axis_fields,
                default=[c for c in ["Age_Ind"] if c in axis_fields],
                key="pivot_cols_duck",
            )
        with c3:
            value_sel = st.multiselect(
                "Values",
                value_fields,
                default=[c for c in ["Death_Count", "Death_Claim_Amount"] if c in value_fields],
                key="pivot_vals_duck",
            )
        with c4:
            agg_choice = st.selectbox("Aggregation", ["sum", "mean", "count", "min", "max", "nunique"], index=0, key="pivot_agg_duck")
            ratio_rate_sel = st.multiselect(
                "Ratio/rate columns",
                ratio_rate_fields,
                default=[],
                key="pivot_ratio_rate_duck",
            )
    ...
    if pivot_apply:
        ...
        safe_ratio_rate = unique_preserve_order([c for c in ratio_rate_sel if c in ratio_rate_fields])

        if not safe_vals and not safe_ratio_rate:
            st.warning("Choose at least one value field or one ratio/rate column.")
        else:
            base = build_pivot_base_duckdb(
                ...
                tuple(safe_vals),
                agg_choice,
                tuple(safe_ratio_rate),
                cat_frozen,
                num_frozen,
            )
            metric_fields = [c for c in safe_vals if c in base.columns] + [c for c in safe_ratio_rate if c in base.columns]
...
def render_analysis_tab(
    ...
    if source_field not in available_columns:
        st.info(f"{human(display_field)} is not present in this predicted parquet file.")
        return
    ...
    with settings_col:
        ...
        if source_field in NUM_COLUMNS:
            max_bin = 10 if source_field in {"Issue_Year", "Issue_Age", "Attained_Age", "Duration"} else 5
        ...
        default_bars = [c for c in ["Death_Count"] if c in numeric_fields]
        ratio_rate_fields = ratio_rate_options_present(available_columns)
        line_metric_options = ratio_rate_fields + [
        ...
        default_line_metrics = [m for m in ["A/E (Count)", "A/P (Count)"] if m in ratio_rate_fields]
        ...
        non_ae_agg = st.selectbox(
            "Aggregation for non-ratio line metrics",
            ["sum", "mean"],
            index=0,
        ...
        merged_for_split_options = active_cat_filters
        if split1:
    ...
    if apply_chart:
        merged_cat_filters = active_cat_filters
...
filter_state_version = "preliminary-default-filters-v1"

if (
    "data_path" not in st.session_state
    or st.session_state.get("data_path") != data_path
    or st.session_state.get("filter_state_version") != filter_state_version
):
...
with tab_issue_year:
    safe_render_tab("Issue Year", lambda: render_analysis_tab(data_path, data_nrows, available_columns, active_cat_filters, active_num_ranges, "Issue Year", "Issue_Year", "Issue_Year"))
with tab_issue:

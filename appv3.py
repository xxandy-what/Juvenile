...
FRIENDLY = {
    ...
    "Predicted_Deaths": "Predicted Deaths - Count",
}
...
FIELD_NOTES = {
    ...
    "Predicted_Deaths": "Predicted deaths from the selected full-refit model.",
}
...
def can_build_filter_field(col: str, available_columns: List[str]) -> bool:
    if col in available_columns:
        return True
    derived_requirements = {
        "Issue_Age_Group": "Issue_Age",
        "Duration_Group": "Duration",
        "Attained_Age_Group": "Attained_Age",
        "Face_Amount_Band_Label": "Face_Amount_Band",
    }
    required = derived_requirements.get(col)
    return required in available_columns if required else False
...
def ae_requirements(metric: str) -> Tuple[str, str, float]:
    ...
    if metric == "A/P (Count)":
        return "Death_Count", "Predicted_Deaths", 1.0
    if metric == "P/E (Count)":
        return "Predicted_Deaths", "ExpDth_VBT2015_Cnt", 1.0
    if metric == "Actual Rate / 1000":
        return "Death_Count", "Policies_Exposed", 1000.0
    if metric == "Expected Rate / 1000":
        return "ExpDth_VBT2015_Cnt", "Policies_Exposed", 1000.0
    if metric == "Predicted Rate / 1000":
        return "Predicted_Deaths", "Policies_Exposed", 1000.0
...
def build_pivot_base_duckdb(
    ...
    if add_measures:
        ...
        if {"Death_Count", "Predicted_Deaths"}.issubset(helper_aliases):
            g["A/P (Count)"] = safe_div(g[helper_aliases["Death_Count"]], g[helper_aliases["Predicted_Deaths"]])
        if {"Predicted_Deaths", "ExpDth_VBT2015_Cnt"}.issubset(helper_aliases):
            g["P/E (Count)"] = safe_div(g[helper_aliases["Predicted_Deaths"]], g[helper_aliases["ExpDth_VBT2015_Cnt"]])
        if {"Death_Count", "Policies_Exposed"}.issubset(helper_aliases):
            g["Actual Rate / 1000"] = 1000.0 * safe_div(g[helper_aliases["Death_Count"]], g[helper_aliases["Policies_Exposed"]])
        if {"ExpDth_VBT2015_Cnt", "Policies_Exposed"}.issubset(helper_aliases):
            g["Expected Rate / 1000"] = 1000.0 * safe_div(g[helper_aliases["ExpDth_VBT2015_Cnt"]], g[helper_aliases["Policies_Exposed"]])
        if {"Predicted_Deaths", "Policies_Exposed"}.issubset(helper_aliases):
            g["Predicted Rate / 1000"] = 1000.0 * safe_div(g[helper_aliases["Predicted_Deaths"]], g[helper_aliases["Policies_Exposed"]])
...
def infer_line_axis_label(line_metrics: List[str]) -> str:
    if line_metrics and all(m in {"A/E (Count)", "A/E (Amount)", "A/P (Count)", "P/E (Count)"} for m in line_metrics):
        return "Lines (ratio)"
...
def infer_line_tickformat(line_metrics: List[str]) -> str:
    ratio_metrics = {"A/E (Count)", "A/E (Amount)", "A/P (Count)", "P/E (Count)"}
...
def render_welcome_tab(source_name: str, data_path: str, available_columns: List[str]) -> None:
    ...
    st.markdown(
        """
        **Basic flow**
        1. Use **Pivot table** for cross-tab summaries with A/E, A/P, P/E, and rate output.
        2. Use the study tabs to build charts from the predicted parquet file.
        3. In each study tab, use the right-side dropdown filters to refine only that chart.
        """
    )
    ...
    st.markdown(
        """
        **Predicted file**
        - The notebook writes `script/model_outputs/session4_scored_full_refit.parquet`.
        - The file is the full-scope scored data after the selected model is refit.
        - It includes `Predicted_Deaths`, `Actual_to_Predicted`, `Actual_to_VBT`, `Predicted_to_VBT`, and rate diagnostics.
        """
    )
...
def render_pivot_tab(
    ...
    st.caption("Build a pivot table directly from the full predicted parquet output.")

    cat_frozen = freeze_cat_filters(active_cat_filters)
    num_frozen = freeze_num_ranges(active_num_ranges)
    working_count = filtered_row_count_query(path, nrows, cat_frozen, num_frozen)
    if working_count == 0:
        st.info("No rows are available in the predicted parquet output.")
        return
    ...
    with st.form("pivot_form"):
        ...
        with c4:
            ...
            add_measures = st.checkbox("Add ratio/rate columns", value=True, key="pivot_add_ae_duck")
    ...
    if pivot_apply:
        ...
        if not safe_vals and not add_measures:
            st.warning("Choose at least one value field, or keep Add ratio/rate columns checked.")
        else:
...
def render_graph_filter_block(
    ...
    for col in [c for c in GRAPH_FILTER_FIELDS if can_build_filter_field(c, available_columns)]:
        ...
        # Only keep this filter if it truly narrows the dataset.
        if opts_str and set(selected) != set(opts_str):
            graph_filters[col] = selected
...
def render_analysis_tab(
    ...
    with settings_col:
        ...
        default_line_metrics = [m for m in ["A/E (Count)", "A/P (Count)"] if m in AE_OPTIONS]
        ...
        active_graph_filters = graph_filter_dict
...
welcome_tab, tab_pivot, tab_obs, tab_issue_year, tab_issue, tab_dur, tab_att, tab_face = st.tabs(
    [
        "Welcome",
        "Pivot table",
        "Observation Year",
